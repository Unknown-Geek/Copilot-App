...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("docgen.generateDocs", () =>
      generateDocsCommand(context)
    ),
    vscode.commands.registerCommand("docgen.translateDocs", () =>
      translateDocsCommand(context)
    ),
    vscode.commands.registerCommand("docgen.exportDocs", () =>
      exportDocsCommand(context)
    ),
    vscode.commands.registerCommand("docgen.analyzeCode", () =>
      analyzeCodeCommand(context)
    ),
    vscode.commands.registerCommand("docgen.analyzeGitHubRepo", () =>
      analyzeGitHubRepoCommand(context)
    ),
    vscode.commands.registerCommand("docgen.loginGitHub", () =>
...
async function generateDocsCommand(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    // Show in webview
    const content =
      typeof response.documentation === "string"
...
async function translateDocsCommand(context: vscode.ExtensionContext) {
  ...
  try {
    const textToTranslate =
      currentDoc || vscode.window.activeTextEditor?.document.getText() || "";
    ...
    vscode.window.showInformationMessage(
      `Documentation translated to ${selected.label}` +
        (response.detected_language
          ? ` (from ${response.detected_language})`
          : "")
    );
 ...
 */
async function analyzeGitHubRepoCommand(context: vscode.ExtensionContext) {
  // Prompt for repository URL
  const repoUrl = await vscode.window.showInputBox({
    prompt: "Enter GitHub repository URL or owner/repo",
    placeHolder:
      "e.g., microsoft/vscode or https://github.com/microsoft/vscode",
    validateInput: (value) => {
      if (!value) {
...
/**
 * Login to GitHub command
 */
async function loginGitHubCommand(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    if (response.auth_url) {
      ...
      if (choice === "Open GitHub") {
        vscode.env.openExternal(vscode.Uri.parse(response.auth_url));
        vscode.window.showInformationMessage(
